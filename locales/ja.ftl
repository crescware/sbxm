...
cli-files-action-help = 行う操作 (addで宣言を足す、lsで一覧する、rmで宣言を外す)
cli-files-path-help = addでは宣言するhost file、rmでは外す宣言の配置先
...
status-files-modified-note = Sandboxの中で変更された宣言fileがあります。宣言fileを配置し直しても、そのfileは置き換えません。置き換えを強制する前に、必要な内容を退避してください。
