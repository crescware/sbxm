...
warning-sandbox-tags-kept = Gitが置き換えを断ったため、{ $sandbox } は次のtagを自分の側のまま残しました。
...
error-sandbox-repository-unwritable = hostから { $sandbox } のrepositoryへ書き込めませんでした。
...
error-no-local-projects = --localで追加した案件がありません。
...
remediation-sandbox-repository-over-ssh = このhostからSandboxへsshでつながるかを確かめてください。
remediation-sandbox-origin-refused = Sandboxの中で、更新を断ったもの（repositoryのhookなど）を確かめてから、もう一度実行してください。
...
remediation-no-local-projects = 先に、このhostにあるGit repositoryを--localで追加してください。
...
remediation-host-commit-unreachable = Sandboxをhostのrepositoryと同期してから、もう一度実行してください。hostへ届いたcommitは、Sandboxを消しても残ります。
...
save-done = { $project } のcommitを { $repository } のrefs/sbx/{ $namespace } へ保存しました。
save-unchanged = { $repository } には { $project } のcommitがすべて保存済みです。
save-nothing = { $project } にはまだ保存するbranchもtagもありません。
save-branches-untouched = hostのrepositoryのbranchとtagには触れていません。置き換えたrefや消えたrefの前の先端はarchiveへ退避し、自動では消しません。
auto-save-done = 先に { $project } のcommitを、{ $repository } のrefs/sbx/{ $namespace } へ保存しました。
auto-save-failed = 先に { $project } のcommitをhostのrepositoryへ保存できなかったため、最後に保存したあとにcommitしたものはSandboxにだけあります。
...
legend-sent-refused = Gitが更新を断り、Sandboxの側が残ったrefです。理由は下に示します
...
select-save-heading = どの案件のcommitを保存しますか?
