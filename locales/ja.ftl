...
error-origin-upstream-missing = checkout中のbranchにupstreamがないため、commitはSandboxの中にしかありません。
error-origin-commit-unpushed = checkout中のbranchに、upstreamが持っていないcommitがあります。
...
remediation-origin-upstream-missing = 残すbranchをoriginへ公開してから、もう一度実行してください。
remediation-origin-commit-unpushed = 残すcommitをoriginへpushしてから、もう一度実行してください。
