...
error-github-secret-missing = Sandbox { $sandbox } にはDocker Sandboxesの { $service } service向けのtokenが登録されていないため、repositoryへaccessできません。
error-github-credential-rejected = Sandbox { $sandbox } が { $repository } へ提示したcredentialを、GitHubが受け付けませんでした。
...
error-secret-still-registered = { $sandbox } 向けのtokenの登録は、削除を要求したあとも一覧に残ったままです。
...
github-token-scopes = 対象repositoryをread/writeできるfine-grained personal access tokenを発行します。必要な権限はContents read/writeとMetadata readです。tokenはDocker Sandboxesのproxyに留まり、Sandboxへは入りません。
remediation-github-secret-missing = { github-token-scopes } 次のcommandでこのSandbox向けに登録してください。tokenは対話で訊かれるため、shell historyには残りません。そのあと同じcommandをもう一度実行してください。
remediation-github-custom-secret-shadowed = そのcustom secretは残しても届かないため削除し、代わりにservice向けにtokenを登録してください。
remediation-sandbox-secret-not-applied = Docker Sandboxesは作成した各Sandboxへ GH_TOKEN を入れます。このSandbox向けにtokenを登録すると即時に反映されるので、登録してから同じcommandをもう一度実行してください。それでも変数が無い場合は、Sandboxを削除して作り直してください。
remediation-secret-still-registered = 登録が残ると、存在しないSandbox宛のtokenを預けたままになります。削除してから、同じcommandをもう一度実行してください。
remediation-github-credential-rejected = { github-token-scopes } tokenが有効で、このrepositoryへのaccess権を持つことを確かめてから、このSandbox向けに登録し直してください。次のcommandで現在の登録を消し、tokenを改めて登録します。そのあと同じcommandをもう一度実行してください。
cause-github-custom-secret-shadowed = { $env } を運ぶcustom secretが登録されていますが、Docker Sandboxesは { $env } を組み込みの { $service } serviceのために予約しているため、そのplaceholderはSandboxへ届きません
cause-github-credential-rejected = proxyが差し替えるのは { $service } service向けに登録したtokenです。期限切れのtoken、このrepositoryへのaccess権がないtoken、登録の欠落は、いずれもここで現れます
...
add-next-secret = このSandbox向けにGitHub tokenを登録します。tokenはcommandが対話で訊きます。
...
destroy-target-secret = Sandbox { $sandbox } 向けにDocker Sandboxesの { $service } serviceへ登録したtoken
