...
remediation-bundle-transfer-incomplete = Sandboxにbundleを置く空きがあること、このversionのDocker SandboxesがSandbox内で実行するcommandへ標準入力を渡すことを確かめてから、もう一度実行してください。
...
status-files-apply-note = hostで変わった宣言file、またはこのSandboxへまだ配置していない宣言fileがあります。次で配置します。
status-files-modified-note = Sandboxの中で変更された宣言fileがあります。apply --filesはそのfileを置き換えません。変更をhostへ持ち帰るにはfiles pullを使い、--forceで置き換える前には必要な内容を退避してください。
...
legend-file-updated = sbxmが配置したあとにhostのfileが変わりました。次のapply --filesで配置されます
