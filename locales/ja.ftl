...
error-origin-upstream-missing = branch { $reference } にupstreamがないため、commitはSandboxの中にしかありません。
error-origin-commit-unpushed = { $reference } には、{ $upstream } が持っていないcommitが { $count } 件あります。
