...
remediation-declared-file-conflict = 両者を確認したうえで、sbxm apply --files を実行すると宣言fileでSandbox側を置き換えられます。
...
remediation-secret-still-registered = 登録が残ると、この案件の次の登録が重複として拒否されます。{ $command } で削除してから、同じcommandをもう一度実行してください。
...
remediation-worktrees-not-reducible = worktreeを減らすことは、そこにcheckoutされているものを消すことです。再実行の副作用として行いません。sbxm destroy を使ってください。
