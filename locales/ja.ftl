...
error-initial-provisioning-pending = { $project } には初回構築の未完了intentがあり、暗黙に再開することはありません。
...
cause-template-id-differs = runtimeはこのtemplateのidを { $observed } と報告していますが、検証済みimageは { $expected } を作ります
...
guidance-next-repair-pending = この案件は初回構築が始まったまま完了していないため、すでに固定したgenerationへ復旧します。
guidance-next-repair-incomplete = 初回構築の成果物が一部欠けているため、案件を開く前に復旧します。
...
guidance-next-then-status = 復旧は固定済みgenerationへ戻します。世代交代がまだ要るかどうかは、そのあとにもう一度診断すると分かります。
