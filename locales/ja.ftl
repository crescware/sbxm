...
cli-apply-all-help = --filesと併せて、1案件ではなく登録済みのすべての案件へ宣言fileを配置します
cli-apply-worktrees-help = 追加後のmanaged worktree総数 (1〜32、現在より減らせません)

cli-files-about = すべてのSandboxへ配置するhost fileを宣言し、一覧し、宣言を外します
cli-files-action-help = 行う操作 (addで宣言を足す、lsで一覧する、rmで宣言を外す、pullでSandboxで変わったcopyを取り出す)
cli-files-path-help = addでは宣言するhost file、rmとpullでは宣言の配置先
cli-files-project-help = pullで、取り出す案件のowner/repository形式の登録ID
cli-files-dest-help = addで、Sandboxのhomeからの配置先 (省略時はhomeからの相対path)
cli-fetch-about = 案件のSandboxのcommitを、hostのrepositoryの専用の名前空間へ保存します
cli-fetch-project-help = 取り出す案件のowner/repository形式の登録ID
cli-send-about = --localで追加した案件のSandboxへ、hostのrepositoryのbranchとtagを送ります
cli-send-project-help = 送り先にする案件の登録ID（local/<name>）
cli-sync-about = --localで追加した案件のSandboxと、hostのrepositoryを同期します
cli-sync-project-help = 同期する案件の登録ID（local/<name>）
...
error-name-without-local = --nameは--localで追加するrepositoryの名前であるため、--localと併せて指定してください。
...
error-invalid-local-repository-path = { $value } はこのhostの正規化された絶対pathではありません。
...
error-file-not-in-sandbox = { $sandbox } の { $destination } にはfileがありません。
...
error-initial-provisioning-pending = { $project } には初回構築の未完了intentがあります。
...
error-initial-provisioning-snapshot-changed = { $path } の固定snapshotが、作られた時点のbyte列と一致しなくなったため使用できません。
error-initial-provisioning-baseline-ambiguous = { $project } は宣言fileのbaselineを記録しておらず、現在のsandboxが現在のglobal configと十分に一致しないため、安全に復元できません。
error-initial-provisioning-unobservable = { $project } の初回構築が完了しているかは、Sandboxが動いていない間は観測できません。
...
error-declared-file-transfer-incomplete = { $sandbox } が { $destination } のために受け取った内容が宣言fileと一致しないため、置き換えませんでした。
error-bundle-transfer-incomplete = { $sandbox } が受け取った内容がhostのrepositoryのbundle全体ではなかったため、置き換えませんでした。
error-sandbox-repository-unreadable = hostから { $sandbox } のrepositoryを読めませんでした。
error-host-repository-empty = { $repository } には、Sandboxへ送るbranchもtagもありません。
error-send-requires-local = { $project } はGitHubから追加した案件であり、そのSandboxはGitHubから自分で取得します。
error-sync-requires-local = { $project } はGitHubから追加した案件であり、そのSandboxはGitHubと自分でfetchとpushをします。
error-host-repository-missing = この案件を登録したhostのrepositoryが、{ $repository } にもうありません。
error-project-inside-repository = 案件のdirectoryが、登録するrepositoryの中の { $path } に作られます。
...
error-sandbox-token-env-unusable = Sandbox { $sandbox } の { $path } からGitHub tokenの環境変数を安全に設定できません。
error-no-github-token = { $project } はこのhostにあるrepositoryから追加した案件であり、入れ替えるGitHub tokenを使いません。
...
error-host-commit-unreachable = hostのrepositoryから到達できないcommitがあります。hostのbranchにもtagにも、このSandboxから保存したものにも含まれていません。
...
error-origin-host-repository-unreadable = この案件のoriginの役を持つhostのrepositoryを読めなかったため、commitが残るかを判定できません。
...
error-protection-not-confirmed = { $project } が入力されなかったため、何も削除しませんでした。
...
error-sandbox-workspace-not-empty = orphanなworkspace directory { $path } が空でないため、安全に再利用できる成果物として扱えません
...
error-external-command-directory-missing = { $program } を実行するdirectoryがありません。
...
error-external-command-output-unstored = commandの出力をこのhostへ残せません
...
error-external-command-input-unavailable = commandへの入力を開けません
...
cause-declared-file-changed-while-placing = 配置の扱いを決めたあとでfileが変わりました
cause-sandbox-copy-changed-while-reading = 読んでいる間にSandbox側の内容が変わりました
cause-host-file-changed-while-deciding = 差分を示したあとでhostのfileが変わりました
...
cause-path-not-utf8 = pathが有効なUTF-8ではありません
...
cause-initial-provisioning-snapshot-changed = 固定したsnapshotが、作成後に変わりました
cause-initial-provisioning-baseline-ambiguous = sandbox内の宣言fileが現在のglobal configと一致せず、壊れたbuildなのか完成後に追加された宣言なのかを区別できる記録済みbaselineがありません
cause-initial-provisioning-unobservable = Sandboxは { $state } であり、中を見るとSandboxが起動します
cause-credential-helper-differs = sandboxは既に別のcredential helperを設定しています: { $observed }
cause-token-env-unexpected-content = fileにsbxmが認識しない内容があります
cause-token-env-unreadable = fileを読み取れませんでした
cause-github-secret-ambiguous = これらのhostを覆う登録が { $count } 件あるため、提示するtokenを決められません
...
cause-template-id-differs = runtimeはこのtemplateのidを { $observed } と報告していますが、検証済みarchiveが宣言するのは { $expected } です
cause-template-id-absent = runtimeはこのtemplateのidを報告しないため、検証済みimageとの対応を確認できません
cause-sandbox-absent = この名前のSandboxが存在しません
...
cause-sandbox-workspace-not-empty = directoryに、説明のつかないentryが残っています
...
cause-head-unobservable = HEADを読めなかったか、何も答えませんでした
...
cause-archive-index-lists-no-manifests = { $entry } がmanifestsを列挙していません
cause-archive-index-digest-unusable = { $entry } はmanifestを { $observed } で指していますが、これはdigestではありません
...
remediation-declared-file-transfer-incomplete = このversionのDocker SandboxesがSandbox内で実行するcommandへ標準入力を渡すかを確かめてから、もう一度実行してください。
remediation-bundle-transfer-incomplete = このversionのDocker SandboxesがSandbox内で実行するcommandへ標準入力を渡すかを確かめてから、もう一度実行してください。
remediation-sandbox-repository-unreadable = このhostからSandboxへsshでつながるかを確かめてください。
remediation-host-repository-empty = hostのrepositoryで一度commitしてから、もう一度実行してください。
remediation-send-requires-local = 代わりにSandboxへ入り、そこでGitを使ってGitHubから取得してください。
remediation-sync-requires-local = 代わりにSandboxへ入り、そこでGitを使ってGitHubとやり取りしてください。
remediation-project-inside-repository = repositoryの外、たとえばrepositoryを置いたdirectoryで、もう一度実行してください。
remediation-host-repository-missing = repositoryをそのpathへ戻してから、もう一度実行してください。移したままにする場合は、この案件を削除し、新しい場所のrepositoryを登録し直してください。
...
remediation-github-secret-ambiguous = このSandboxに対してこれらのhostを覆う登録が複数あり、どのtokenを提示すべきか決まりません。この案件のものでない登録を消して1件にしてから、同じcommandをもう一度実行してください。
remediation-github-credential-rejected = { github-token-scopes } tokenが有効で、このrepositoryへ今も届くことを確かめてください。以下のcommandで既存のplaceholderを維持したままcustom secretを更新し、同じcommandをもう一度実行してください。Sandboxを作り直す必要はありません。
...
remediation-sandbox-not-created = 案件を開き、最初の実行でSandboxを構築します。
remediation-sandbox-not-running = Sandboxを起動してから、もう一度実行してください。
remediation-sandbox-workspace-missing = workspaceはhost側の空のmount点であり、repositoryはSandboxの中にあるため、このdirectoryには案件のものが何もありません。案件を復旧すると、このdirectoryを作り直し、作ったことを示します。自分でdirectoryを作っても構いません。
...
remediation-worktree-tracked-changes-host = Sandboxへ入り、そのworktreeで何が変更されたか確認してください。残す変更はcommitしてhostのrepositoryへ保存し、残さない変更は内容を確認してから戻してください。worktreeがcleanになった後、もう一度実行してください。
remediation-worktree-untracked-paths = 上に列挙された未追跡pathを確認してください。残すfileはadd・commit・pushし、残さないfileは内容を確認してから移すか削除してください。
remediation-worktree-untracked-paths-host = 上に列挙された未追跡pathを確認してください。残すfileはaddしてcommitし、hostのrepositoryへ保存してください。残さないfileは内容を確認してから移すか削除してください。
...
remediation-git-operation-in-progress-host = そのworktreeが示す案内に従い、操作をcontinueまたはabortしてください。残す変更はcommitしてhostのrepositoryへ保存し、Git操作中でなくなってから、もう一度実行してください。
...
remediation-origin-host-repository-unreadable = hostのrepositoryが、案件を追加したときのpathにまだあることを確かめてから、もう一度実行してください。
...
remediation-protection-not-confirmed = もう一度実行し、promptが示す案件のIDを入力してください。
...
remediation-initial-provisioning-baseline-ambiguous = 新しく宣言したfileはapplyで明示的に配置するか、sandboxを自分で確認してください。repairは元のbuildが持っていた宣言を推測しません。
remediation-initial-provisioning-unobservable = openで案件を起動して接続してください。sbxmは中を見るためだけにSandboxを起動しません。
...
remediation-file-declaration-not-removable = { $path } から { $destination } のfiles宣言を自分で削除してください。sbxmは、ほかの記述を変えずに編集できると確かめられない設定を書き換えることはしません。
remediation-file-not-declared = 宣言fileの一覧から、外す配置先を確かめてください。
remediation-file-already-declared = このfileで置き換える場合は、先にその宣言を外してください。
remediation-file-destination-required = Sandboxのhome directory内の配置先を指定してください。
remediation-config-not-rewritable = { $path } へ `{ $declaration }` を自分で書き足してから、もう一度実行してください。sbxmは、ほかの記述を変えずに編集できると確かめられない設定を書き換えることはしません。
...
add-field-host-repository = hostのrepository (Host repository)
...
column-ref = ref (REF)
column-archived = 退避先 (ARCHIVED AS)
...
files-removed = { $destination } の宣言を { $path } から外しました。Sandboxへ配置済みのfileはそのまま残ります。
files-none-declared = 宣言fileはありません。
files-apply-hint = 宣言は保存しましたが、まだどのSandboxにも配置していません。登録済みのすべての案件へ配置するには次を実行します。
warning-file-looks-like-credential = { $source } は認証情報を持つfileによく使われる名前です。
files-pull-same = { $project } の { $destination } は { $source } と同じ内容です。
files-pull-diff = { $project } の { $destination } は { $source } と次の差分があります。
files-pull-adopted = { $project } の { $destination } の内容を { $source } へ採用しました。
files-pull-spread-hint = ほかの案件には前の内容が残っています。登録済みのすべての案件へ配置するには次を実行します。
files-pull-kept = { $source } はそのまま残しました。{ $project } 側の内容もそのままです。
files-pull-undecided = 何も採用していません。Sandbox側の内容を採用するかは、対話端末で実行して選んでください。
fetch-done = { $project } のcommitを { $repository } のrefs/sbx/{ $namespace } へ保存しました。
fetch-unchanged = { $repository } には { $project } のcommitがすべて保存済みです。
fetch-nothing = { $project } にはまだ保存するbranchもtagもありません。
fetch-branches-untouched = hostのrepositoryのbranchとtagには触れていません。置き換えたrefや消えたrefの前の先端はarchiveへ退避し、自動では消しません。
auto-save-done = 先に { $project } のcommitを、{ $repository } のrefs/sbx/{ $namespace } へ保存しました。
auto-save-failed = 先に { $project } のcommitをhostのrepositoryへ保存できなかったため、最後のfetchのあとにcommitしたものはSandboxにだけあります。
send-done = { $repository } のbranchとtagを、{ $project } のSandboxへ送りました。
send-unchanged = { $project } のSandboxは、{ $repository } のbranchとtagをすでにすべて持っています。
send-worktrees-untouched = Sandboxのworktreeとbranchはそのままです。取り込むには、Sandboxの中でorigin/<branch>をmergeするかrebaseしてください。
sync-done = { $project } を { $repository } と同期しました。
sync-unchanged = { $project } と { $repository } は、すでに同期しています。
sync-heading-host = hostのrepository
sync-heading-sandbox = Sandboxのorigin
sync-left-as-it-was = 作成も更新もされなかったrefは、Gitがそのままにしました。そのcommitは { $repository } のrefs/sbx/{ $namespace } に残してあります。
sync-refused-because = Gitが { $reference } を断りました: { $reason }
sync-sandbox-untouched = Sandboxのworktreeとbranchはそのままです。hostの変更を取り込むには、Sandboxの中でorigin/<branch>をmergeするかrebaseしてから、もう一度同期してhostへ届けてください。
...
apply-all-stopped-note = 停止中のSandboxはそのままにしました。案件を開いてSandboxを起動してから、宣言fileを適用してください。
...
legend-reachable = originのref、またはhostへ保存したものが、最後に観測できた時点でこのcommitへ到達できました
legend-unreachable = originのどのrefも、hostへ保存したものも、最後に観測できた時点でこのcommitへ到達できませんでした
legend-unobservable = remoteがこのcommitへ到達できるかを確認できませんでした。値の括弧内に理由を示します
legend-placed = Sandboxへ書き込みました
legend-ref-created = refを作りました
legend-ref-updated = 前の先端から先へ進めました
legend-ref-replaced = 前の先端を含まない先端へ置き換えました。前の先端はarchiveへ退避しています
legend-ref-deleted = Sandboxに無くなったrefです。前の先端はarchiveへ退避しています
legend-sent-created = 初めてSandboxへ届いたrefです
legend-sent-updated = hostのrepositoryの先端へ動いたrefです
legend-sent-removed = hostのrepositoryに無くなったため、Sandboxのoriginからも消えたbranchです
legend-reflect-created = hostのrepositoryに作ったbranchかtagです
legend-reflect-updated = Sandboxの先端まで進めたhostのbranchです
legend-reflect-behind = hostのbranchより遅れているだけのSandboxのbranchです。失うものはありません
legend-reflect-diverged = hostとSandboxで、それぞれ相手に無いcommitを持つbranchです
legend-reflect-checked-out = hostのrepositoryでcheckoutしているため、Gitが動かさなかったbranchです
legend-reflect-exists = 同じ名前で、hostのrepositoryでは別の先を指しているtagです
legend-reflect-refused = 下に示す理由で、Gitが更新を断ったrefです
...
destroy-confirm-prompt = 削除を確認するため { $project } と入力してください
confirm-retry-prompt = { $project } と一致しません。続けるには入力し直してください。Escでcancelします (残り { $attempts } 回)
...
repair-plan-observations-heading = 観測したartifact
repair-plan-actions-heading = repairの範囲
repair-observation-sandbox = Sandbox
repair-observation-workspace = 中立workspace
repair-observation-declared-file = 宣言file { $destination }
...
repair-observation-worktrees = managed worktree
repair-action-no-change = 変更しません。案件はfreshまたはreadyのため、作業するときはopenを使ってください。
repair-action-record-intent = 最初のrepair mutationより前に復旧intentを記録します。
repair-action-reuse-image = 案件と世代のlabelが一致する既存imageを再利用します。
repair-action-build-image = 固定したsnapshotからimageをbuildします。
repair-action-reuse-template = runtime idまで検証済みimageと一致する既存Templateを再利用します。
repair-action-load-template = 検証済みarchiveからTemplateをloadします。
repair-action-restore-workspace = 消えた空の中立workspace directoryを作り直します。
repair-action-create-sandbox = Sandboxを作成します。
repair-action-start-sandbox = 最初の内部検査により、停止中Sandboxを起動します。
repair-action-provision-interior = 未観測のSandbox内部へ接続し、宣言file、Git設定、credential helper、bare repository、managed worktreeの全構築工程を実行します。
repair-action-place-declared-file = { $destination } へ宣言fileを配置します。
repair-action-configure-identity = Sandbox内のGit identityを設定します。
repair-action-configure-credential-helper = Sandbox内のcredential helperを設定します。
repair-action-create-bare-repository = 共有bare repositoryを作成します。
repair-action-create-worktree = { $path } にmanaged worktreeを作成します。
repair-action-clear-intent = 読み取り専用の完成確認後にだけintentを消します。
...
select-fetch-heading = どの案件からcommitを取り出しますか?
select-send-heading = hostのbranchをどの案件へ送りますか?
select-sync-heading = どの案件をhostのrepositoryと同期しますか?
select-files-pull-heading = どの案件からfileを取り出しますか?
...
legend-apply-applied = 宣言fileをSandboxへ書き込みました
legend-apply-unchanged = 宣言fileはすべてSandboxに同じ内容で既にありました
legend-apply-stopped = Sandboxが停止していたため、何も配置していません
legend-apply-not-created = Sandboxがまだありません。初回構築で宣言fileが配置されます
legend-apply-failed = 宣言fileを適用できませんでした。理由は診断が示します
status-project-section = 案件 (PROJECT)
status-files-section = 宣言file (DECLARED FILES)
status-files-apply-note = hostで変わった宣言file、このSandboxへまだ配置していない宣言file、またはSandboxから無くなった宣言fileがあります。次で配置します。
status-files-modified-note = Sandboxの中で変更された宣言fileがあります。宣言fileを配置し直しても、そのfileは置き換えません。変更はhostへ持ち帰れます。置き換えを強制する前には、必要な内容を退避してください。
status-worktrees-section = worktree (WORKTREES)
status-next-heading = 次の一手 (Next):
...
status-item-host-repository = hostのrepository (Host repository)
...
legend-file-unchanged = sbxmが前回配置した内容と同じです。記録がなければhostのfileと同じです
legend-file-updated = sbxmが配置したあとにhostのfileが変わりました。宣言fileを配置し直すと置かれます
legend-file-unplaced = この案件へはまだ配置していません
legend-file-unreadable = hostのfileをそのままでは配置できません。理由は診断が示します
legend-file-modified = sbxmが配置したあとにSandboxの中で変更されています
legend-file-unrecorded = hostのfileと異なり、sbxmが配置した記録もありません
...
progress-saving-to-host = { $project } のSandboxのcommitを、hostのrepositoryへ保存します。
...
guidance-next-open-pending = 初回構築が完了していません。openが記録済みのgenerationから再開して接続します。
guidance-next-open-incomplete = 接続に必要な状態が一部欠けています。openが確認できた不足を補って接続します。
guidance-next-rebuild-pending = 世代交代が始まったまま完了していないため、同じ世代交代を完了します。
guidance-next-rebuild-changed = Dockerfileが適用済みgenerationと異なるため、新しいgenerationとして適用します。
guidance-next-then-status = 復旧は記録済みgenerationを完成させます。その世代の成果物が一つも無い場合だけ現在のDockerfileを採用します。世代交代がまだ要るかどうかは、そのあとにもう一度診断すると分かります。
...
prompt-files-pull-heading = { $source } をSandbox側の内容で置き換えますか？
prompt-files-pull-keep = hostのfileを残す
prompt-files-pull-adopt = Sandbox側の内容を採用する
prompt-files-apply-heading = 登録済みの { $count } 件の案件へ、宣言fileを今すぐ配置しますか？
prompt-save-to-host-heading = Sandboxのcommitをhostのrepositoryへ保存してから続けますか？
prompt-save-to-host-save = hostのrepositoryへ保存して続ける
prompt-save-to-host-stop = 保存せずに止める
prompt-files-apply-now = 今すぐ配置する
prompt-files-apply-later = あとで配置する
