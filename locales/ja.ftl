...
error-worktree-tracked-changes = { $worktree } には、追跡対象ファイルのcommitされていない変更があります。
error-worktree-untracked-paths = { $worktree } には、Gitが保存しない未追跡fileが { $count } 件あります。
error-git-operation-in-progress = { $worktree } はGit操作の途中です ({ $operation })。
error-origin-commit-unreachable = { $reference } が持つ { $commit } は、remoteのどのrefからも到達できません。
error-origin-missing = Sandbox内のrepositoryにoriginが設定されていないため、{ $reference } をoriginと照合できません。
error-origin-refresh-failed = originをrefreshできなかったため、{ $commit } を持つ { $reference } を照合できませんでした。
error-origin-advertisement-invalid = originのref一覧を読み取れなかったため、{ $commit } を持つ { $reference } を照合できませんでした。
error-origin-object-missing = { $commit } がSandbox内のrepositoryに無いため、{ $reference } を照合できませんでした。
error-origin-observation-unobservable = { $sandbox } のoriginへ到達できなかったため、commitをそこから回収できるかを判定できません。
...
error-worktree-inventory-unobservable = { $sandbox } のworktree一覧を読み取れなかったため、保存されていない作業があるかを判定できません。
...
error-ignored-paths-unobservable = { $worktree } にGitが追跡しない無視対象のpathがあるかを判定できませんでした。
error-reflog-unobservable = { $worktree } にreflogだけが覚えているcommitがあるかを判定できませんでした。
error-remote-configuration-unobservable = { $worktree } のremoteとbranchのupstream設定を一覧できませんでした。
