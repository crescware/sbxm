...
remediation-declared-file-conflict = Compare the two yourself, then run sbxm apply --files to replace the file in the sandbox with the declared one.
...
remediation-secret-still-registered = A registration that stays behind makes the next registration for this project refused as a duplicate. Remove it with { $command }, then run the same command again.
...
remediation-worktrees-not-reducible = Removing a worktree deletes whatever is checked out in it, so it is not something a re-run does. Use sbxm destroy for that.
...
security-ssh-agent-exposed-remediation = A sandbox keeps whatever the daemon gave it when it was created, so restarting the daemon is not enough on its own. Stop every running sandbox, start the daemon again from a shell where SSH_AUTH_SOCK is unset, run { $command } to drop this one, then run the command again to build it clean.
