...
remediation-bundle-transfer-incomplete = Check that the sandbox has room for the bundle and that this version of Docker Sandboxes passes standard input to a command run inside a sandbox, then run the command again.
...
status-files-apply-note = Some declared files changed on the host or were never placed in this sandbox. Place them with:
status-files-modified-note = Some declared files were changed inside the sandbox. apply --files leaves them alone. Bring a change back to the host with files pull, or save what you need before replacing it with --force.
...
legend-file-updated = the host file changed after sbxm placed it; the next apply --files places it
