...
remediation-sandbox-workspace-missing = The workspace is an empty mount point on the host, and the repository lives inside the sandbox, so the directory holds nothing of the project. Building the project again creates the directory and reports that it did. Creating the directory yourself works too.
