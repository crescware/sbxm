...
cli-repair-about = Repair an interrupted or incomplete first provisioning without changing its fixed target generation
...
error-initial-provisioning-pending = { $project } has an unfinished first-provisioning intent, and it is never resumed implicitly.
...
error-initial-provisioning-input-changed = The global file input for { $project } changed while first provisioning was in progress.
...
cause-initial-provisioning-input-changed = the declared path, destination, or content digest differs from the saved intent
...
cause-template-id-differs = the runtime reports id { $observed } for this template, but the verified image builds to { $expected }
...
remediation-initial-provisioning-input-changed = Restore the global file declarations and their contents, or inspect the partial Sandbox manually. sbxm will not overwrite a changed input during repair.
...
guidance-next-repair-pending = The first provisioning of this project started and did not finish, so recover it to the generation it already fixed.
guidance-next-repair-incomplete = Part of the first provisioning result is missing, so recover it before opening the project.
...
guidance-next-then-status = Recovery goes back to the fixed generation. Diagnose the project again afterwards to see whether a generation change is still needed.
