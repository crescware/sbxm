...
error-worktree-tracked-changes = { $worktree } has changes to tracked files that are not committed.
error-worktree-untracked-paths = { $worktree } has { $count } untracked files that Git does not store.
error-git-operation-in-progress = { $worktree } is in the middle of a Git operation ({ $operation }).
error-origin-commit-unreachable = { $reference } holds { $commit } that no ref on the remote reaches.
error-origin-missing = { $reference } cannot be checked against the origin because the sandbox's repository has no origin configured.
error-origin-refresh-failed = { $reference } holding { $commit } could not be checked because the origin could not be refreshed.
error-origin-advertisement-invalid = { $reference } holding { $commit } could not be checked because the origin's ref list could not be read.
error-origin-object-missing = { $reference } holding { $commit } could not be checked because that commit is missing from the sandbox's repository.
error-origin-observation-unobservable = The origin of { $sandbox } could not be reached to check whether commits can be recovered from it.
...
error-worktree-inventory-unobservable = The worktree inventory of { $sandbox } could not be listed, so whether it holds unsaved work is unknown.
error-worktree-status-unobservable = Whether { $worktree } holds uncommitted or untracked changes could not be determined.
error-git-operation-unobservable = Whether { $worktree } is in the middle of a Git operation could not be determined.
error-local-refs-unobservable = The HEAD, branch, and upstream of { $worktree } could not be resolved, so whether its commits can be recovered from the origin is unknown.
error-ignored-paths-unobservable = Whether { $worktree } has ignored paths that Git does not track could not be determined.
error-reflog-unobservable = Whether { $worktree } holds commits that only its reflog remembers could not be determined.
error-remote-configuration-unobservable = The remotes and branch upstream configuration of { $worktree } could not be listed.
