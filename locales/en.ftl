...
cli-fetch-about = Save the commits of a project sandbox into its host repository, under a namespace of their own
cli-fetch-project-help = Registered owner/repository ID of the project to fetch from
cli-send-about = Send the branches and tags of the host repository to the sandbox of a project added with --local
cli-send-project-help = Registered local/<name> ID of the project to send to
...
error-bundle-transfer-incomplete = What reached { $sandbox } was not the whole bundle of the host repository, so nothing was replaced.
...
error-send-requires-local = { $project } was added from GitHub, and its sandbox fetches from GitHub itself.
...
remediation-bundle-transfer-incomplete = Check that this version of Docker Sandboxes passes standard input to a command run inside a sandbox, then run the command again.
remediation-sandbox-repository-unreadable = Check that this host can connect to the sandbox over SSH.
remediation-host-repository-empty = Commit at least once in the host repository, then run the command again.
remediation-send-requires-local = Open the sandbox and fetch from GitHub with Git there instead.
...
fetch-done = Saved the commits of { $project } into { $repository } under refs/sbx/{ $namespace }.
fetch-unchanged = { $repository } already holds every commit of { $project }.
fetch-nothing = { $project } has no branch or tag to save yet.
fetch-branches-untouched = Your branches and tags in the host repository were left as they were. A tip that was replaced or deleted is kept under archive and is never removed automatically.
auto-save-done = Saved the commits of { $project } into { $repository } under refs/sbx/{ $namespace } first.
auto-save-failed = The commits of { $project } could not be saved into the host repository first, so anything committed since the last fetch is still only in the sandbox.
send-done = Sent the branches and tags of { $repository } to the sandbox of { $project }.
send-unchanged = The sandbox of { $project } already has every branch and tag of { $repository }.
send-worktrees-untouched = The worktrees and branches in the sandbox were left as they were. Merge or rebase onto origin/<branch> inside the sandbox to take the changes in.
...
select-fetch-heading = Which project do you want to fetch commits from?
select-send-heading = Which project do you want to send the host's branches to?
