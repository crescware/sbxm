...
repair-plan-no-change = No mutation: the project is already fresh or ready; use prepare or open as appropriate.
repair-plan-verify = Re-observe the declared target and existing artifacts.
repair-plan-reuse = Reuse only artifacts whose project and generation labels match.
repair-plan-provision = Repair the missing first-provisioning steps and verify their post-conditions.
repair-plan-clear-intent = Clear the intent only after read-only completion verification.
repair-plan-record-intent = Record the recovery intent before the first repair mutation.
