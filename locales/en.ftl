...
error-github-secret-missing = The sandbox { $sandbox } has no token registered for the { $service } service of Docker Sandboxes, so it cannot reach the repository.
error-github-credential-rejected = GitHub did not accept the credential the sandbox { $sandbox } presented for { $repository }.
...
error-secret-still-registered = A token registration for { $sandbox } was still listed after it was removed.
...
error-protection-not-confirmed = The sandbox name was not entered exactly as { $sandbox }, so nothing was deleted.
...
github-token-scopes = Issue a fine-grained personal access token that can read and write this repository: Contents read and write plus Metadata read. The token stays with the Docker Sandboxes proxy and never enters the sandbox.
remediation-github-secret-missing = { github-token-scopes } Register it for this sandbox with the command below; it prompts for the token so the value stays out of your shell history. Then run the same command again.
remediation-github-custom-secret-shadowed = Remove that custom secret so it does not linger, and register the token for the service instead.
remediation-sandbox-secret-not-applied = Docker Sandboxes fills GH_TOKEN in every sandbox it creates. Register the token for this sandbox, which takes effect at once, then run the same command again. If the variable is still absent afterwards, remove the sandbox and build it anew.
remediation-secret-still-registered = A registration that stays behind keeps a token attached to a sandbox that no longer exists. Remove it, then run the same command again.
remediation-github-credential-rejected = { github-token-scopes } Check that the token is still valid and has access to this repository, then register it again for this sandbox: remove the current registration and register the token anew with the commands below. Then run the same command again.
cause-github-custom-secret-shadowed = a custom secret carrying { $env } is registered, but Docker Sandboxes reserves { $env } for its built-in { $service } service, so the sandbox never receives that placeholder
cause-github-credential-rejected = the proxy injects the token registered for the { $service } service; an expired token, a token without access to this repository, or a missing registration all end here
...
remediation-protection-not-confirmed = Type the sandbox name exactly as shown, then run this command again.
...
add-next-secret = Register the GitHub token for this sandbox. The command prompts for the token.
...
destroy-target-secret = the token registered for the { $service } service of Docker Sandboxes for the sandbox { $sandbox }
...
rebuild-confirm-prompt = Type the sandbox name to confirm the rebuild
