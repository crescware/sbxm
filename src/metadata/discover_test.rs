...
fn discovery_returns_every_project_in_canonical_order() {
    ...
    for (owner, repository) in [("zeta", "zulu"), ("alpha", "alfa"), ("alpha", "another")] {
        write_project(
            dir.path(),
            repository,
    ...
    let ids: Vec<String> = found
        ...
        .collect();
    assert_eq!(ids, vec!["alpha/alfa", "alpha/another", "zeta/zulu"]);
}
...
fn discovery_looks_at_the_documented_shape_only() {
    ...
    fs::create_dir_all(dir.path().join("owner/nested.project/.sbxm")).unwrap();
    fs::write(
        dir.path().join("owner/nested.project/.sbxm/project.yaml"),
        render(&attached("owner", "nested")),
    )
    .unwrap();
    fs::create_dir_all(dir.path().join("plain-directory")).unwrap();
...
fn a_symlinked_project_directory_is_not_followed() {
    ...
    std::os::unix::fs::symlink(&real, dir.path().join("linked-repo.project")).unwrap();
...
fn one_broken_project_stops_the_whole_listing() {
    ...
    write_project(dir.path(), "broken-repo", "version: 2\n");
...
fn a_project_stored_outside_its_derived_path_is_refused() {
    let dir = tempfile::tempdir().unwrap();
    let base = AbsoluteBasePath::new(dir.path()).unwrap();
    // metadataはexample-repoの案件だが、別の名前のdirectoryへ置かれている。
    write_project(
        dir.path(),
        "other-repo",
        &render(&attached("example-org", "example-repo")),
...
fn distinct_projects_are_not_a_conflict() {
    ...
    let projects: Vec<DiscoveredProject> =
        [("example-org", "example-repo"), ("other", "other-repo")]
            .into_iter()
