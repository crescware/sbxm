//! `fetch`のparser非依存command-line解釈。
...
impl CommandLine {
    pub(crate) fn syntax(builder: &Builder) -> Result<CommandSyntax> {
        Ok(builder.command("fetch", "cli-fetch-about")?.arg(
            ArgumentSyntax::value("project", builder.text("cli-fetch-project-help")?)
    ...
    pub(crate) fn interpret(
        ...
        CommandLineValues::optional_project(arguments, prompt, "sbxm fetch")
    }
}
