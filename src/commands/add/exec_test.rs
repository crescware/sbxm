...
fn add_args(git_identity: Option<GitIdentity>) -> Args {
    Args {
...
fn saved_identity(location: &ConfigLocation) -> Option<GitIdentity> {
    config::load(location)
        .expect("the configuration is readable")
        .settings()
        .git_identity
...
#[test]
fn the_first_interactive_add_asks_for_the_identity_and_saves_what_was_entered() {
    let (_dir, location) = home();
    let context = context(&location, None, tty());
...
#[test]
fn the_host_is_offered_as_a_candidate_rather_than_taken_as_the_value() {
    let (_dir, location) = home();
    let context = context(&location, None, tty());
...
#[test]
fn a_host_that_declares_nothing_leaves_an_empty_field_rather_than_failing() {
    let (_dir, location) = home();
    let context = context(&location, None, tty());
    ...
    .expect("an unobservable host does not stop the run");
...
#[test]
fn a_saved_default_is_used_without_asking_again() {
    let (_dir, location) = home();
    ...
    for interactivity in [tty(), non_tty()] {
        ...
        .expect("a saved default needs no prompt");
...
#[test]
fn a_declared_identity_decides_the_run_without_asking_or_saving() {
    ...
    for interactivity in [tty(), non_tty()] {
        for saved in [None, Some(identity())] {
            ...
            .expect("a declared identity needs no prompt");
...
#[test]
fn a_run_that_can_neither_ask_nor_read_a_default_refuses_before_creating_anything() {
    let (_dir, location) = home();
    let context = context(&location, None, non_tty());
    let mut prompt = ScriptedIdentityPrompt::typing("Never Asked", "never@example.com");
    ...
    .expect_err("there is nothing that decides the identity");
...
#[test]
fn cancelling_the_identity_prompt_saves_nothing() {
    let (_dir, location) = home();
    let context = context(&location, None, tty());
...
#[test]
fn a_value_that_cannot_be_a_git_identity_is_refused_rather_than_saved() {
