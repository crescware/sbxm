...
impl CommandLine {
    pub(crate) fn syntax(builder: &Builder) -> Result<CommandSyntax> {
        Ok(builder.command("send", "cli-send-about")?.arg(
            ArgumentSyntax::value("project", builder.text("cli-send-project-help")?)
    ...
    pub(crate) fn interpret(
        ...
        CommandLineValues::optional_project(arguments, prompt, "sbxm send")
    }
}
