...
impl CommandLine {
    pub(crate) fn syntax(builder: &Builder) -> Result<CommandSyntax> {
        Ok(builder.command("sync", "cli-sync-about")?.arg(
            ArgumentSyntax::value("project", builder.text("cli-sync-project-help")?)
    ...
    pub(crate) fn interpret(
        ...
        CommandLineValues::optional_project(arguments, prompt, "sbxm sync")
    }
}
