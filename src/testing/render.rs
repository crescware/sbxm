...
/// 色を出さない描画。既定の比較対象とする。
pub fn plain(document: &Document, locale: Locale) -> String {
    render(document, locale, StreamPolicy::plain())
}

/// 指定した条件での描画。
pub fn render(document: &Document, locale: Locale, policy: StreamPolicy) -> String {
    let mut buffer: Vec<u8> = Vec::new();
    {
        let mut renderer = Renderer::new(&mut buffer, policy);
        renderer.write(&Catalog::new(locale), document);
    }
    String::from_utf8(buffer).expect("the renderer writes UTF-8 except for external output")
}
