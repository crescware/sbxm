...
#[test]
fn a_parent_directory_must_be_an_absolute_path_that_exists() {
    let dir = temp_dir();
    ProjectParent::at(dir.path()).expect("an existing directory can hold a project");
    ...
    for declared in [
        ...
        let error = ProjectParent::at(&declared).expect_err("{declared:?} cannot hold a project");
        assert_eq!(
            error.first_id(),
...
#[test]
fn a_parent_directory_rejects_an_existing_regular_file() {
    let dir = temp_dir();
    let file = dir.path().join("not-a-directory");
    fs::write(&file, b"x").expect("write file");
    let error = ProjectParent::at(&file).expect_err("files cannot hold a project");
...
fn project_paths_follow_the_documented_layout() {
    let dir = temp_dir();
    let parent = ProjectParent::at(dir.path()).expect("valid parent directory");
