...
pub fn observe_for_mutation(
    ...
    if !origin_configured(host, sandbox_name, &git_dir)? {
        return Ok(unobservable(UnobservableReason::OriginMissing));
    }

    let fetch = repository::refresh_origin(host, sandbox_name, &git_dir, None)
        .map_err(|error| reclassify(&error, sandbox_name))?;
    match sandbox::inner_exit_code(&fetch) {
        Some(0) => {}
        Some(_) => return Ok(unobservable(UnobservableReason::RefreshFailed)),
        None => return Err(unobservable_command(&fetch, sandbox_name)),
    }

    let tips = match origin_tips(host, sandbox_name, &git_dir)? {
        Ok(tips) => tips,
    ...
    for commit in unique_commits {
        match reaching_origin_refs(host, sandbox_name, &git_dir, commit)? {
            Ok(origins) => {
...
fn origin_configured(
    ...
    match sandbox::inner_exit_code(&outcome) {
        ...
        Some(1) => Ok(false),
        _ => Err(unobservable_command(&outcome, sandbox_name)),
    }
}
...
fn origin_tips(
    ...
    if sandbox::inner_exit_code(&outcome) != Some(0) {
        return Err(unobservable_command(&outcome, sandbox_name));
    }
...
fn reaching_origin_refs(
    ...
    match sandbox::inner_exit_code(&outcome) {
        ...
        None => return Err(unobservable_command(&outcome, sandbox_name)),
    }
...
fn reclassify(error: &Error, sandbox_name: &str) -> Error {
    let mut diagnostic = Diagnostic::new(
        ErrorId::OriginObservationUnobservable,
        msg!(
            "error-origin-observation-unobservable",
            sandbox = sandbox_name
        ),
    )
    .remediation(msg!("remediation-origin-observation-unobservable"));
...
fn unobservable_command(outcome: &CommandOutcome, sandbox_name: &str) -> Error {
    Error::single(
        Diagnostic::new(
            ErrorId::OriginObservationUnobservable,
            msg!(
