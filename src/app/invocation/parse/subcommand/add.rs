...
impl Add {
    pub(super) fn spec(builder: &Builder) -> Result<ClapCommand> {
        Ok(builder
            ...
            .arg(
                Arg::new("repository")
                    .required(true)
                    .value_name(project_arg::CLONE_URL_VALUE_NAME)
    ...
    pub(super) fn parse(matches: &ArgMatches) -> Result<Args> {
        let repository = project_arg::required_clone_url(matches)?;
