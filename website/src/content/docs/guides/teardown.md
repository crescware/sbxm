...
A commit that is not on the origin is kept once it is saved into the host repository with [`sbxm fetch`](../../reference/cli/fetch/). When such commits are the only thing stopping the teardown, sbxm offers to save them and continue.
